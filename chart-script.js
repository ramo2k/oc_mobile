...
function processData() {
    ...
    if (!yearDate && !startDate && !endDate) {
        alertElement.innerText = "Veuillez saisir une date.";
        alertElement.style.color = 'red';
        alertIcon.className = 'fas fa-times-circle'; // display error icon
        return;
    }
    ...
    if (yearDate && !startDate && !endDate) {
        ...
        console.log(`Filtering data from ${startOfYear} to ${endOfYear}`); 

        filteredData = fetchedData.filter(item => {
            const itemDate = moment(item.heure).format('YYYY-MM-DD');
            console.log(`Checking item date ${itemDate}`); 
            return moment(itemDate).isSameOrAfter(startOfYear) && moment(itemDate).isSameOrBefore(endOfYear);
        });    

    } else if (startDate && endDate && !yearDate) {
        const formattedStartDate = moment(startDate).format('YYYY-MM-DD');
        const formattedEndDate = moment(endDate).format('YYYY-MM-DD');
    
        console.log(`Filtering data from ${formattedStartDate} to ${formattedEndDate}`);
